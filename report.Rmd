...
First, I looked to see if there were differences in the AMA between the different joints studied; independent of the species. In order to check this, I performed permutation tests to test for differences between joints. To account for multiple comparisons, a test-wise $\alpha$ level of 0.005 was used, which leads to a family-wise $\alpha$ level of 0.05.
...
To determine the power of the permutation tests conducted above, I ran a simulation. The distribution we are using has a spread parameter $\kappa$ which governs the spread between joints with a lower $\kappa$ resulting in a lower spread. I ran the simulation four times while changing parameters $\kappa$ and sample size $n$. I repeated the test 30 times for the 4 $\kappa$ parameters: 5, 20, 50, and 100 as well as the 3 $n$ values: 10, 50, and 100. As previously mentioned, each test contains 10,000 permutations, so a total of 3.6 million tests were ran. The resulting cumulative density functions (CDF) are shown in *figure 1* below:  
